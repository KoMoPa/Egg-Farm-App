...
export default function Form08WelfareRecords({ farmId, farmName, barnNumber, monthYear }) {
  ...
  return (
    ...
    const handleSubmit = async (e) => {
      ...
      // Step 2: Prepare daily records data (Page 1) - ONLY for days with data
      const dailyRecords = Object.values(dayData)
                .filter(day => {
                    // Only include days that have at least one field filled in
                    return day.barnTempHi || day.barnTempLo || day.exteriorTemp ||
      day.floorsChecked || day.wallsFansCeilingChecked || day.manureChecked ||
      day.beddingUsed || day.chemicalsUsed || day.ammoniaLevel
                })
                .map(day => ({
        farm_id: farmId,
      audit_id: auditId,
      ...
      // Step 3: Prepare equipment inspection records data (Page 2) - ONLY for days with data
      const equipmentRecords = Object.values(dayData)
                .filter(day => {
                    // Only include days that have at least one inspection field filled in
                    return day.routineHenEquip1stInitial || day.routineHenEquip1stDaily ||
      day.routineHenEquip2ndInitial || day.routineHenEquip2ndDaily ||
      day.overallAppearance || day.generalSound || day.abnormalBehavior ||
      day.signsOfDisease || day.injuredBirds || day.trappedBirds ||
      day.deadBirds || day.feedWaterAvailable || day.equipmentOperating ||
      day.amenitiesCondition || day.layFacilityEnvironment
                })
                .map(day => ({
        farm_id: farmId,
      audit_id: auditId,
